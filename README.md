...
该库已经打算弃用了，最初想使用github的总结来保证自己一个非常规整的学习习惯（因为是公共库，虽然很少人看到），该库在一段时间内满足了我的需要。
但随着学习的逐渐深入，发现渐渐的有一系列缺陷：

- 分类很繁琐，有时候并没有想清楚分类到哪里，后期要改的话，战线太长
- 搜索回顾不够方便
- 模板非常单一
- 公式插入的话，md的天然劣势（虽然可以通过一些稍微麻烦些的办法解决）
- 难以增加其他学科学习知识点，例如C++、算法。又得开新库
- 有的知识点可能需要比较长时间，需要设立各个阶段，但现有情况下难以区分

偶尔间我发现了Notion，这个软件基本解决了我以上的问题，所以决定将所有后续个人总结迁移到Notion中，该库也不会在更新。
