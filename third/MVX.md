...
## MVP监督控制器（Supervising Controller）
...
### MVP缺点
- 复杂性：MVP 架构可能比较复杂，需要编写大量代码来管理视图和业务逻辑之间的交互。
- 可测试性：MVP 架构不太容易测试，因为需要编写较多的测试代码来测试各个部分之间的交互。
- 过度解耦：MVP 架构可能会过渡解耦，使得架构变得过于分离，导致代码难以理解和维护。
- 会增加很多方法数
...
3. 可扩展性：MVVM架构可以使应用的扩展变得更加简单，因为视图模型层可以使用数据绑定来绑定视图状态和视图模型层的数据，这可以在不更改视图的情况下更新视图状态。
